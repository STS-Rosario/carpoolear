        ...
        return passengerApi
            ...
            .then((response) => {
                ...
                if (trip.request !== 'send') {
                    if (
                        globalStore.getters['trips/currentTrip'] &&
                        globalStore.getters['trips/currentTrip'].id === trip.id
                    ) {
                        globalStore.commit(
                            'trips/' +
                                types.TRIPS_CURRENT_REMOVE_PASSENGER_BY_ID,
                            user.id
                        );
                    }
                    globalStore.commit(
                    ...
                    if (trip.user.id !== myUser.id) {
                        ...
                        globalStore.commit(
                            'myTrips/' + types.MYTRIPS_REMOVE_PASSENGER_TRIP,
                            trip.id
                        );
                    } else {
                        globalStore.commit(
...
const mutations = {
    ...
    [types.PASSENGER_REMOVE_PENDING_PAYMENT](state, data) {
        ...
        if (
            ...
            state.pendingPaymentRequests.length
        ) {
